...
app.get("/api/tournaments/:tournamentId/sync-battlelogs", async (req, res) => {
  ...
  item.battleTime +
  "_" +
  (item.event?.id || "noevent") +
  "_" +
  (item.battle?.mode || "nomode") +
  "_" +
...
app.get("/api/tournaments/:tournamentId/update-bots", async (req, res) => {
  try {
    ...
    snapshot.forEach(doc => {
      ...
      // chance qu'un bot soit inactif
      const inactivityChance = Math.random();

      // 35% de chance de ne rien gagner
      if (inactivityChance < 0.35) {
        return;
      }
      ...
      // type de bot
      const botType = data.botType || "medium";

      // faible
      if (botType === "weak") {
        gain = Math.floor(Math.random() * 4) + 1;
      }

      // moyen
      else if (botType === "medium") {
